...
# ----------------------------------------------------------------------
# Plot 1: spectral decomposition (no red "significant" points)
# ----------------------------------------------------------------------
...
plt.xlabel(r"Eigenvalue $E_n$", fontsize=label_fs)
plt.ylabel(r"$|\langle \phi_n | \psi_0 \rangle|^2$", fontsize=label_fs)
plt.title("Rydberg Spectral Decomposition", fontsize=title_fs)
plt.yscale("log")
...
# ----------------------------------------------------------------------
# Plot 2: entropy vs eigenvalue (no red "significant" points)
# ----------------------------------------------------------------------
...
plt.figure(figsize=(9, 5))
plt.plot(e_vals_r, entropy_all, "o", color="tab:blue", ms=4, alpha=0.6, label="All eigenstates")
if len(selected_indices) > 0:
    plt.plot(selected_energies, entropy_selected, "o",
             color="gold", ms=8, mec="k", mew=0.8, label="Selected")

plt.xlabel(r"Eigenvalue $E_n$", fontsize=label_fs)
plt.ylabel("Entanglement Entropy (bits)", fontsize=label_fs)
plt.title("Entanglement Entropy vs Eigenvalue", fontsize=title_fs)
